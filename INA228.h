...
//#define INA228_ADDRESS 0x40 // 000 AGND


// define ina registers
#define CONFIG_REG 0x0
#define ADCCONFIG_REG 0x1
#define SHUNT_CAL_REG 0x2
#define SHUNT_TEMPCO_REG 0x3
...
union ADCCONFIG_REGISTER_U
{
    uint16_t all;
    struct ADCCONFIG_REGISTER bit;
};

//--------------Address 0x2---------------------------------
struct SHUNT_CAL_REGISTER
{
    uint16_t CURRLSB : 15;        // 0-14
    uint16_t RESERVED : 1;       // 15
};
union SHUNT_CAL_REGISTER_U
{
    uint16_t all;
    struct SHUNT_CAL_REGISTER bit;
};

//--------------Address 0x3---------------------------------
struct SHUNT_TEMPCO_REGISTER
{
    uint16_t TEMPCO : 14;  // 0-13
    uint16_t RESERVED : 2; // 14-15
};
union SHUNT_TEMPCO_REGISTER_U
{
    uint16_t all;
    struct SHUNT_TEMPCO_REGISTER bit;
};
...
struct DIAG_ALRT_REGISTER
{
    ...
    uint16_t MATHOF : 1;                   // 1-0
    uint16_t CHARGEOF : 1;                 // 11-2
    uint16_t ENERGYOF : 1;                 // 11-
    uint16_t APOL : 1;                    // 1-0
    uint16_t SLOWALERT : 1;               // 1-0
    uint16_t CNVR : 1;                    // 11-2
    uint16_t ALATCH : 1;                  // 1-0
};
...
// All  registers
typedef struct INA228Reg
{
    union CONFIG_REGISTER_U reg0;
    union ADCCONFIG_REGISTER_U reg1;
    union SHUNT_CAL_REGISTER_U reg2;
    union SHUNT_TEMPCO_REGISTER_U reg3;
...
class INA228
{
    ...
    boolean getENERGYOF();
    boolean getAPOL();
    boolean getSLOWALERT();
    boolean setCNVR(byte);
    boolean setALATCH(byte);

    // SOVL 0xc
    boolean setSOVL(uint16_t value);
    boolean setSOVL_f(float value);

    // SUVL 0xd
    boolean setSUVL(uint16_t value);
    boolean setSUVL_f(float value);
